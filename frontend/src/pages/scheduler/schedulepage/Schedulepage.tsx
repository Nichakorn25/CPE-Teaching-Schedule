...
const Schedulepage: React.FC = () => {
  ...
  const applyFilters = () => {
    ...
    const filtered = scheduleData.map(dayData => {
      // Filter subCells based on tags and search
      const filteredSubCells = dayData.subCells?.filter(subCell => {
        // Apply tag filters
        const tagMatch = filterTags.length === 0 || filterTags.every(tag => {
          switch (tag.type) {
            case 'teacher':
              return subCell.classData.teacher.toLowerCase().includes(tag.value.toLowerCase());
            case 'studentYear':
              // Check against AcademicYearID from API data
              const scheduleFromOriginal = originalScheduleData.find((original: any) => 
                original.ID === subCell.scheduleId
              );
              
              if (scheduleFromOriginal) {
                // ใช้ any เพื่อหลีกเลี่ยง TypeScript error
                const academicYearId = (scheduleFromOriginal.OfferedCourses?.AllCourses as any)?.AcademicYear?.AcademicYearID;
                
                if (academicYearId) {
                  // เปรียบเทียบตัวเลขกับตัวเลข
                  const yearIdString = academicYearId.toString();
                  const match = yearIdString === tag.value;
                  console.log(`🎓 Student Year Filter: AcademicYearID ${academicYearId} -> ${yearIdString}, Filter: ${tag.value}, Match: ${match}`);
                  return match;
                }
                
                // Alternative: check Level field
                const level = (scheduleFromOriginal.OfferedCourses?.AllCourses as any)?.AcademicYear?.Level;
                if (level && level !== 'เรียนได้ทุกชั้นปี') {
                  // ถ้า Level เป็น "ปีที่ X" ให้แปลงเป็นตัวเลขเปรียบเทียบ
                  const yearMatch = level.match(/ปีที่\s*(\d+)/);
                  if (yearMatch) {
                    const levelMatch = yearMatch[1] === tag.value;
                    console.log(`🎓 Student Year Filter (Level): ${level} -> ${yearMatch[1]}, Filter: ${tag.value}, Match: ${levelMatch}`);
                    return levelMatch;
                  }
                  
                  // ถ้าเป็นตัวเลขอยู่แล้ว
                  const directMatch = level === tag.value;
                  console.log(`🎓 Student Year Filter (Direct): ${level}, Filter: ${tag.value}, Match: ${directMatch}`);
                  return directMatch;
                }
              }
              
              // ตรวจสอบจาก subCell.classData.studentYear เป็นทางเลือกสุดท้าย
              const studentYearMatch = subCell.classData.studentYear === tag.value;
              console.log(`🎓 Student Year Filter (SubCell): ${subCell.classData.studentYear}, Filter: ${tag.value}, Match: ${studentYearMatch}`);
              return studentYearMatch;
            default:
              return true;
          }
        });
        ...
        const searchMatch = !searchValue || 
          subCell.classData.teacher.toLowerCase().includes(searchValue.toLowerCase());

        return tagMatch && searchMatch;
      }) || [];

      // Return dayData with filtered subCells
      return {
        ...dayData,
        subCells: filteredSubCells
      };
    }).filter(dayData => {
      // Only include days that have matching subCells or show all days if no filters
      return filterTags.length === 0 && !searchValue ? true : 
             (dayData.subCells && dayData.subCells.length > 0) ||
             filterTags.some(tag => tag.type === 'day' && tag.value === dayData.day);
    });
